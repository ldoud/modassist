...
import com.github.ldoud.modassist.base.PrimaryModStatBaseTest;
...
class PrimaryModStatXmlTest extends PrimaryModStatBaseTest {
    ...
    @Override
    protected void assertPrimaryStat(CharacterName toon, ModType modType, ModStat primaryModStat, String expectedStatValue) {
        Node modXml = data.getMod(toon, modType);
        String xpath = XPATH_STAT.replace("${statName}", primaryModStat.toString());
        ...
        String asserting = modXml.selectSingleNode("@character").getText()+"'s "+ primaryModStat.toString();
